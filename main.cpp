...
int main() {
  ...
  double tempo_otimizado =
      double(fim_otimizado - inicio_otimizado) / CLOCKS_PER_SEC;

  printf("Tempo Algoritmo Padrao: %.4f s\n", tempo_padrao);
  printf("Tempo Algoritmo Otimizado: %.4f s\n\n", tempo_otimizado);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    std::vector<ProdutoRecomendado> recs =
        recomendarProdutos(idx_cliente, k, &compras, S, nClientes, nProdutos);

    printf("Top %d recomendacoes para o cliente '%s':\n", k, codigo);
    if (recs.empty()) {
      printf("  Nao ha dados suficientes para recomendar produtos a este "
             "cliente.\n");
    } else {
